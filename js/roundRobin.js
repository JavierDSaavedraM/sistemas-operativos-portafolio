...
document.getElementById("btn-run-sched").addEventListener("click", function() {
  ...
  if (activeTab.id == "tab-caso1"){ 
    simData.processes = [ 
      { pid:1, arrival: 0, burst: 4, remaining: 4}, 
      { pid:2, arrival: 0, burst: 3, remaining: 3},
      { pid:3, arrival: 0, burst: 5, remaining: 5}
    ];
    simData.quantum = 2;
  }
    ...
    simData.processes = [ 
      ...
      { pid:2, arrival: 0, burst: 4, remaining: 4},
      { pid:3, arrival: 0, burst: 2, remaining: 2},
      { pid:4, arrival: 0, burst: 1, remaining: 1},
    ];
    simData.quantum = 2;
  }
  else if (activeTab.id == "tab-caso3"){ 
    simData.processes = [ 
      { pid:1, arrival: 0, burst: 5, remaining: 10}, 
      { pid:2, arrival: 0, burst: 4, remaining: 4}, 
      { pid:3, arrival: 0, burst: 9, remaining: 9}, 
      { pid:4, arrival: 0, burst: 7, remaining: 7}, 
      { pid:5, arrival: 0, burst: 10, remaining: 10} 
    ];
    simData.quantum = 2;
  }
  else if (activeTab.id == "tab-caso4"){ 
    simData.processes = [ 
      { pid:1, arrival: 0, burst: 250, remaining: 250}, 
      { pid:2, arrival: 0, burst: 170, remaining: 170}, 
      { pid:3, arrival: 0, burst: 75, remaining: 75}, 
      { pid:4, arrival: 0, burst: 100, remaining: 100}, 
      { pid:5, arrival: 0, burst: 325, remaining: 325},
      { pid:6, arrival: 0, burst: 50, remaining: 50} 
    ];
    simData.quantum = 100;
  }
    ...
    simData.processes = [ 
      ...
      { pid:2, arrival: 2, burst: 8, remaining: 8}, 
      { pid:3, arrival: 4, burst: 6, remaining: 6}, 
      { pid:4, arrival: 6, burst: 4, remaining: 4}, 
      { pid:5, arrival: 8, burst: 2, remaining: 2}
    ];
    simData.quantum = 4;
  }
  else if (activeTab.id == "tab-caso6"){ 
    simData.processes = [ 
      { pid:1, arrival: 0, burst: 9, remaining: 9}, 
      { pid:2, arrival: 1, burst: 6, remaining: 6}, 
      { pid:3, arrival: 3, burst: 12, remaining: 12}, 
      { pid:4, arrival: 5, burst: 3, remaining: 3}, 
      { pid:5, arrival: 8, burst: 5, remaining: 5}
    ];
    simData.quantum = 3;
  }
  else {
...
function startSched() {
  var quantum   = simData.quantum;
  if (quantum <= 0) return;
  if (!validateSimData()) return;
  var procs = copyProcesses();
  var timeline = runRR(procs, simData.quantum); 
